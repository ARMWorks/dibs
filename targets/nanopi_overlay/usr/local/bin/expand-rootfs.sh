...
ENV_SIZE=32
BL1_SIZE=16
let RESERVED_SIZE=$KERNEL_SIZE+$BL2_SIZE+$ENV_SIZE+$BL1_SIZE+2
let RESERVED_POSITION=$DEVICE_SIZE-$RESERVED_SIZE
if [[ $DEVICE_SIZE -ge 4194303 ]]; then
        let RESERVED_POSITION=$RESERVED_POSITION-1024
fi

let FREE_SIZE=$RESERVED_POSITION-$P2_POSITION-$P2_SIZE

if [[ "$SWAP_SIZE_MB" -gt 0 ]]; then
    MAKE_SWAP=1
    let P3_SIZE=$SWAP_SIZE_MB*2048
    let P3_POSITION=$RESERVED_POSITION-$P3_SIZE
